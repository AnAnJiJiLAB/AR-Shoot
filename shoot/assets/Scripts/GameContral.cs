...
public class GameContral : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if (MonsterFire <= 0) 
		{
			...
			if(Produce == 4)
			{
				Instantiate (Monster, new Vector3 (3, 65, 742), transform.rotation);

			}
			if(Produce == 1)
			{
				Instantiate (Monster2, new Vector3 (3, 65, 742), transform.rotation);
		...
		ScoreShow.GetComponent<GUIText>().text = "" + Score;
		LifeShow.GetComponent<GUIText>().text = "" + Life;
