...
public class GameContral3 : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (MonsterFire <= 0) 
		{
			...
			if(Produce == 4)
			{
				Instantiate (Monster, new Vector3 (-757, 65, -2), transform.rotation);
				
			}
			if(Produce == 1)
			{
				Instantiate (Monster2, new Vector3 (-757, 65, -2), transform.rotation);
