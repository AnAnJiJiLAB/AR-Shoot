...
public class GameContral8 : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (MonsterFire <= 0) 
		{
			...
			if(Produce == 4)
			{
				Instantiate (Monster, new Vector3 (-692, 65, -676), transform.rotation);
				
			}
			if(Produce == 1)
			{
				Instantiate (Monster2, new Vector3 (-692, 65, -676), transform.rotation);
